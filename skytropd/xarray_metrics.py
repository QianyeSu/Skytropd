...
class MetricAccessor:
    def __init__(self, xarray_obj: xr.Dataset):
        ...
        self.metric_property_name = dict(
            ...
            psi_streamfunction=["psi", "Mass streamfunction"],
    ...
    def compute_metrics(self) -> xr.DataArray:
        """Return the requested skytropd metric for this data"""
    ...
    def validate_data(self):
        ...
        if self.metric_name == "psi":
            field_type = str(self.params.get("field_type", "V")).strip().upper()
            if field_type == "PSI":
                property_name = self.metric_property_name["psi_streamfunction"]
    ...
    def xr_edj(self, **params) -> xr.DataArray:
        ...
        **kwargs : optional
        ...
        >>> ds = xr.open_dataset("skytropd/ValidationData/ua.nc")
    ...
    def xr_olr(self, **params) -> xr.DataArray:
        ...
        **kwargs : optional
        ...
        >>> import skytropd as pyt
        >>> import matplotlib.pyplot as plt
        >>> ds = -xr.open_dataset("skytropd/ValidationData/rlnt.nc")
    ...
    def xr_pe(self, **params) -> xr.DataArray:
        ...
        >>> import skytropd as pyt
        >>> import matplotlib.pyplot as plt
        >>> prds = xr.open_dataset("skytropd/ValidationData/pr.nc")
        >>> eds = -xr.open_dataset("skytropd/ValidationData/hfls.nc") / 2510400.0
    ...
    def xr_psi(self, **params) -> xr.DataArray:
        ...
        The :py:class:`Dataset` should contain one variable corresponding to zonal-mean
        meridional wind when ``field_type="V"`` or a precomputed mass streamfunction
        when ``field_type="PSI"``. If multiple non-coordinate variables are in the
        dataset, this method attempts to guess which field corresponds to the requested
        input based on the variable name. The :py:class:`Dataset` should also contain a
        latitude-like dimension and a pressure-like dimension.
        ...
        field_type : {"V", "PSI"}, optional
            Whether the dataset variable should be interpreted as meridional wind or a
        ...
        :py:class:`KeyError`:
            ...
            corresponding to the requested PSI input cannot be detected
        ...
        >>> import skytropd as pyt
        >>> import matplotlib.pyplot as plt
        >>> ds = xr.open_dataset("skytropd/ValidationData/va.nc")
        >>> PSI_metrics = ds.pyt_metrics.xr_psi(method="Psi_500_10Perc")
        >>> PSI_metrics.squeeze().plot.line(x="time", add_legend=False)
        >>> plt.show()
        >>> psi = pyt.TropD_Calculate_StreamFunction(ds.va, ds.lat, ds.lev)
        >>> psi_ds = psi.to_dataset(name="psi")
        >>> PSI_metrics = psi_ds.pyt_metrics.xr_psi(field_type="PSI")
        """
    ...
    def xr_psl(self, **params) -> xr.DataArray:
        ...
        **kwargs : optional
        ...
        >>> pslds = xr.open_dataset("skytropd/ValidationData/psl.nc")
    ...
    def xr_tpb(self, **params) -> xr.DataArray:
        ...
        **kwargs : optional
        ...
        >>> import skytropd as pyt
        >>> import matplotlib.pyplot as plt
        >>> tds = xr.open_dataset("skytropd/ValidationData/ta.nc").rename(ta="T")
        >>> zds = xr.open_dataset("skytropd/ValidationData/zg.nc").rename(zg="Z")
        >>> ds = xr.merge([tds, zds])
