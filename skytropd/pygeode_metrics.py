...
def metrics_dataset(
  ...
  **params
  ...
  property_name = metric_property_name[metric]
  if metric == 'psi' and str(params.get('field_type', 'V')).strip().upper() == 'PSI':
    property_name = metric_property_name['psi_streamfunction']
  var, _ = extract_property(dataset, property_name=property_name, p_axis_status=p_axis_status)
...
def extract_property(
  ...
  if property_name[0] == 'lat':
    ...
    #otherwise look for a matching variable name 
      ...
      elif property_name[0] == 'psi':
        property_name_list = ['psi','streamfunction','massstreamfunction','massoverturningstreamfunction','msf']
...
def pyg_edj(dataset: pyg.Dataset,**params) -> pyg.Dataset:
  ...
  '''TropD Eddy Driven Jet (EDJ) metric
	 ...
	 Returns:
	   EDJ_metrics: :class:Var` with axis :class:`歉amedAxis` Metric (SH latitudes, NH latitudes)
...
def pyg_olr(dataset: pyg.Dataset, **params) -> pyg.Dataset:
  ...
  """TropD Outgoing Longwave Radiation (OLR) metric
	 ...
	 Returns:
	   OLR_metrics: :class:Var` with axis :class:`歉amedAxis` Metric (SH latitudes, NH latitudes)
  Examples
  --------
  >>> import pygeode as pyg
  >>> from pygeode.tutorial import t2 
...
def pyg_pe(dataset: pyg.Dataset, **params) -> pyg.Dataset:
  ...
  ''' TropD Precipitation minus Evaporation (PE) metric
	 ...
	 Returns:
	   PE_metrics: :class:Var` with axis :class:`歉amedAxis` Metric (SH latitudes, NH latitudes)
  Examples
  --------
  >>> import pygeode as pyg
  >>> from pygeode.tutorial import t2 
...
def pyg_psi(dataset: pyg.Dataset,**params) -> pyg.Dataset:
  ...
  Parameters:
	...
	V(lat,lev): zonal-mean meridional wind when ``field_type='V'`` or a
	...
	field_type (str, optional): ``'V'`` for meridional wind input or ``'PSI'`` for
	                            precomputed mass streamfunction input. Default is ``'V'``.

	 Returns:
	   PSI_metrics: :class:Var` with axis :class:`歉amedAxis` Metric (SH latitudes, NH latitudes)
...
def pyg_psl(dataset: pyg.Dataset,**params) -> pyg.Dataset:
  ...
  ''' TropD Sea-level pressure (PSL) metric
	 ...
	 Returns:
	   PSL_metrics: :class:Var` with axis :class:`歉amedAxis` Metric (SH latitudes, NH latitudes)
...
def pyg_stj(dataset: pyg.Dataset,**params) -> pyg.Dataset:
  ...
  '''TropD Eddy Driven Jet (STJ) metric
	 ...
	 Returns:
	   STJ_metrics: :class:Var` with axis :class:`歉amedAxis` Metric (SH latitudes, NH latitudes)
...
def pyg_tpb(dataset: pyg.Dataset,**params) -> pyg.Dataset:
  ...
  ''' TropD Tropopause break (TPB) metric
	 ...
	 Returns:
	   TPB_metrics: :class:Var` with axis :class:`歉amedAxis` Metric (SH latitudes, NH latitudes)
...
def pyg_uas(dataset: pyg.Dataset,**params) -> pyg.Dataset:
  ...
  ''' TropD near-surface zonal wind (UAS) metric
	 ...
	 Returns:
	   UAS_metrics: :class:Var` with axis :class:`歉amedAxis` Metric (SH latitudes, NH latitudes)
